...
import image1 from "../../asset/SVG's/Group 1.svg";
import image2 from "../../asset/SVG's/Group 2.svg";
import image3 from "../../asset/SVG's/Group 3.svg";
import testimonial1 from "../../asset/SVG's/Testimonial 1.png";
import testimonial2 from "../../asset/SVG's/Testimonial2.png";
// MUI 5 stepper start
...
const ProductDetails = () => {
  ...
  const addProductToCartCart = () => {
    if (!authData?.userData?.uid) {
      dispatchSnackbarData({ action: "UPDATE", payload: { message: "Please sign in and try again!", severity: "error", open: true } });
  ...
  useEffect(() => {
    ...
    getDocument(collections.PRODUCTS, documentId)
      .then((res) => {
        setProduct(res);
        setSelectedImage(res?.product?.image?.["0"]);
      })
      .catch((err) => {
        console.log(err);
      });
